...
func TestArrayRepeatedMissingNumberUsingMarkingLogic(t *testing.T) {
	tests := []struct {
		name             string
		input            []int
		expectedMissing  int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Make a copy since marking modifies the array
			inputCopy := make([]int, len(tt.input))
			copy(inputCopy, tt.input)
			
			missing, repeated := dsa_brushup.Array_repeated_missing_number_using_marking_logic(inputCopy)
			
			if missing != tt.expectedMissing {
				t.Errorf("Array_repeated_missing_number_using_marking_logic(%v) missing = %d; want %d", 
					tt.input, missing, tt.expectedMissing)
			}
			
			if repeated != tt.expectedRepeated {
				t.Errorf("Array_repeated_missing_number_using_marking_logic(%v) repeated = %d; want %d", 

...
func TestArraySortColorDutchFlagAlgorithm(t *testing.T) {
	tests := []struct {
		name     string
		input    []int
		expected []int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := dsa_brushup.ArraySortColorDutchFlagAlgorithm(tt.input)
			if !reflect.DeepEqual(result, tt.expected) {
				t.Errorf("ArraySortColorDutchFlagAlgorithm(%v) = %v; want %v", tt.input, result, tt.expected)
			}
		})
	}
}

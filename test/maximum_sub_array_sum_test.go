...
func TestMaximumSubArraySumBruteForce(t *testing.T) {
	tests := []struct {
		name     string
		input    []int
		expected int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := dsa_brushup.Maximum_sub_array_sun_brute_force(tt.input)
			if result != tt.expected {
				t.Errorf("Maximum_sub_array_sun_brute_force(%v) = %d; want %d", tt.input, result, tt.expected)
			}
		})
	}
}
